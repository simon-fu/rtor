...
use fast_socks5::{consts, AuthenticationMethod, ReplyError, Result, SocksError};
use anyhow::Context;
...
impl FromU8 for AuthenticationMethod { 
    #[inline]
    #[rustfmt::skip]
...
impl AsU8 for AuthenticationMethod { 
    #[inline]
    #[rustfmt::skip]
    fn as_u8(&self) -> u8 {
        match self {
...
pub struct Socks5Op<'a, T> {
    inner: &'a mut T,
}

impl <'a, T> Socks5Op<'a, T> 
where
    T: AsyncRead + AsyncWrite + Unpin
{
    ...
    ///
    pub async fn read_request(&mut self) -> Result<(Socks5Command, TargetAddr)> {
    ...
    // pub async fn reply_request(&mut self) -> Result<()> {
